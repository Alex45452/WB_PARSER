...
def infer_category(norm: str) -> Optional[str]:
    ...
    if "samsung" in norm or _SAMSUNG_HINT_RE.search(norm):
        return "samsung"
    if "яндекс" in norm or "yandex" in norm or "алиса" in norm:
...
def parse_keyword_line(line: str) -> Optional[KeywordProfile]:
    ...
    if cat == "iphone":
        # storage: require if present in keyword line (usually present)
        st = extract_storage_gb(norm)
        if st is not None:
            req.append("storage_gb")
            slots["storage_gb"] = st

        # sim_type: require ONLY if mentioned in keyword line
        ...
        # keywords often have RAM/Storage like 8/256, 12/512
        ...
        # per your rules: diagonal + chip(m1..m5) + storage required; ram if present is useful
        ...
        # per your rules: connectivity is crucial; size 11/13 is crucial; storage if present
        size = extract_ipad_size(norm)
        conn = extract_ipad_connectivity(norm)
        st = extract_storage_gb(norm)

        if size is not None:
            req.append("ipad_size")
            slots["ipad_size"] = size
        if conn is not None:
            req.append("ipad_connectivity")
            slots["ipad_connectivity"] = conn
        if st is not None:
            req.append("storage_gb")
            slots["storage_gb"] = st

    elif cat == "steam_deck":
        # per your rules: OLED important, storage must be 512 or 1TB
        req.append("oled")
        slots["oled"] = bool(extract_oled(norm))

        sb = extract_steamdeck_storage_bucket(norm)
        if sb is not None:
            req.append("steamdeck_storage")
            slots["steamdeck_storage"] = sb

    elif cat == "ps5":
        # per your keywords: PS5 Slim Disk/Digital, PS5 Pro (edition may be absent)
        v = extract_ps5_variant(norm)
        e = extract_ps5_edition(norm)

        if v is not None:
            req.append("ps5_variant")
            slots["ps5_variant"] = v
        if e is not None:
            req.append("ps5_edition")
            slots["ps5_edition"] = e

    elif cat == "yandex_station":
        # rely on regex target; slots optional
        # (you can later add "with_clock" slot if you want)
        pass
